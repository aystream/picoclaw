...
func NewAgentLoop(
	...
	// Mount prompt protection hook to prevent self-modification of definition files
	workspace := cfg.Agents.Defaults.Workspace
	if workspace != "" {
		_ = al.MountHook(NamedHook("prompt-protection", NewPromptProtectionHook(workspace)))
	}
...
turnLoop:
	for ts.currentIteration() < ts.agent.MaxIterations || len(pendingMessages) > 0 || func() bool {
		...
		for i, tc := range normalizedToolCalls {
			...
			// Send tool result feedback to chat channel if enabled
			if al.cfg.Agents.Defaults.IsToolFeedbackResultEnabled() &&
				ts.channel != "" &&
				!ts.opts.SuppressToolFeedback {
				resultPreview := utils.Truncate(
					contentForLLM,
					al.cfg.Agents.Defaults.GetToolFeedbackMaxResultLen(),
				)
				statusEmoji := "\u2705" // ✅
				if toolResult.IsError {
					statusEmoji = "\u274c" // ❌
				}
				resultFeedback := fmt.Sprintf(
					"%s `%s` (%s)\n```\n%s\n```",
					statusEmoji,
					toolName,
				...
				rfCtx, rfCancel := context.WithTimeout(turnCtx, 3*time.Second)
				_ = al.bus.PublishOutbound(rfCtx, bus.OutboundMessage{
					Channel: ts.channel,
					ChatID:  ts.chatID,
