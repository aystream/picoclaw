	...
	go func() {
		for {
			if startErr := bh.Start(); startErr != nil {
				logger.ErrorCF("telegram", "Bot handler failed, restarting polling in 5s", map[string]any{
					"error": startErr.Error(),
				})
			} else {
				// bh.Start returned nil — normal shutdown via Stop()
				return
			}
			...
			// Re-create polling and bot handler
			newUpdates, pollErr := c.bot.UpdatesViaLongPolling(c.ctx, &telego.GetUpdatesParams{
				Timeout: 30,
			})
			if pollErr != nil {
				logger.ErrorCF("telegram", "Failed to restart polling", map[string]any{
					"error": pollErr.Error(),
				})
				continue
			}

			newBH, bhErr := th.NewBotHandler(c.bot, newUpdates)
			if bhErr != nil {
			...
			newBH.HandleMessage(func(ctx *th.Context, message telego.Message) error {
				return c.handleMessage(ctx, &message)
			}, th.AnyMessage())

			c.bh = newBH
			logger.InfoC("telegram", "Telegram polling reconnected")
		}
	}()
