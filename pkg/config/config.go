...
type ToolFeedbackConfig struct {
	...
	ShowResult    bool `json:"show_result"     env:"PICOCLAW_AGENTS_DEFAULTS_TOOL_FEEDBACK_SHOW_RESULT"`
	MaxResultLen  int  `json:"max_result_len"  env:"PICOCLAW_AGENTS_DEFAULTS_TOOL_FEEDBACK_MAX_RESULT_LEN"`
}
...
// Also returns true when the DEBUG_TOOLS env var is set to "true".
...
// IsToolFeedbackResultEnabled returns true when tool result summaries should be sent to the chat.
func (d *AgentDefaults) IsToolFeedbackResultEnabled() bool {
	if os.Getenv("DEBUG_TOOLS") == "true" {
		return true
	}
	return d.ToolFeedback.ShowResult
}

// GetToolFeedbackMaxResultLen returns the max result preview length for tool feedback messages.
func (d *AgentDefaults) GetToolFeedbackMaxResultLen() int {
	if d.ToolFeedback.MaxResultLen > 0 {
		return d.ToolFeedback.MaxResultLen
	}
	return 500
}
