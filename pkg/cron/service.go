...
// DefaultJobTTL is the default time-to-live for new cron jobs.
// Jobs without an explicit ExpiresAtMS get this TTL applied automatically.
const DefaultJobTTL = 90 * 24 * time.Hour
...
// removeExpiredJobs removes jobs past their ExpiresAtMS. Must be called under write lock.
func (cs *CronService) removeExpiredJobs() int {
	now := time.Now().UnixMilli()
	kept := cs.store.Jobs[:0]
	removed := 0
	for _, job := range cs.store.Jobs {
		if job.ExpiresAtMS != nil && *job.ExpiresAtMS <= now {
			log.Printf("[cron] removing expired job '%s' (id: %s)", job.Name, job.ID)
			removed++
			continue
		}
...
func (cs *CronService) checkJobs() {
	...
	// Purge expired jobs first
	if n := cs.removeExpiredJobs(); n > 0 {
		if err := cs.saveStoreUnsafe(); err != nil {
			log.Printf("[cron] failed to save store after expiry cleanup: %v", err)
		}
	}
...
func (cs *CronService) AddJob(
	...
	defaultExpiry := now + DefaultJobTTL.Milliseconds()
	job := CronJob{
		ID:       generateID(),
		Name:     name,
		Enabled:  true,
