...
func TestCronService_DefaultTTL(t *testing.T) {
	cs, path := setupService(nil)
	defer os.Remove(path)

	at := time.Now().Add(time.Hour).UnixMilli()
	job, err := cs.AddJob("TTLTest", CronSchedule{Kind: "at", AtMS: &at}, "msg", "ch", "to")
	if err != nil {
		t.Fatalf("AddJob failed: %v", err)
	}

	if job.ExpiresAtMS == nil {
		t.Fatal("New job should have a default ExpiresAtMS set")
	}

	expectedExpiry := time.Now().Add(DefaultJobTTL).UnixMilli()
	diff := *job.ExpiresAtMS - expectedExpiry
	if diff < -5000 || diff > 5000 {
		t.Errorf("ExpiresAtMS should be ~%d days from now, got diff=%dms", int(DefaultJobTTL.Hours()/24), diff)
	}
}

func TestCronService_ExpiredJobsRemoved(t *testing.T) {
	cs, path := setupService(nil)
	defer os.Remove(path)

	// Add a job with expiry in the past
	at := time.Now().Add(time.Hour).UnixMilli()
	job, err := cs.AddJob("ExpiredJob", CronSchedule{Kind: "every", EveryMS: int64Ptr(60000)}, "msg", "ch", "to")
	if err != nil {
		t.Fatalf("AddJob failed: %v", err)
	}
	_ = at
	...
	job.ExpiresAtMS = &pastExpiry
	cs.UpdateJob(job)

	// Add a non-expired job
	_, err = cs.AddJob("ValidJob", CronSchedule{Kind: "every", EveryMS: int64Ptr(60000)}, "msg", "ch", "to")
	if err != nil {
		t.Fatalf("AddJob failed: %v", err)
	}

	if len(cs.ListJobs(true)) != 2 {
		t.Fatalf("Expected 2 jobs before cleanup, got %d", len(cs.ListJobs(true)))
	}

	// Run checkJobs — should remove expired job
	cs.mu.Lock()
	cs.running = true
	cs.mu.Unlock()
	cs.checkJobs()
	...
	if len(jobs) != 1 {
		t.Fatalf("Expected 1 job after cleanup, got %d", len(jobs))
	}
	if jobs[0].Name != "ValidJob" {
		t.Errorf("Expected ValidJob to survive, got %s", jobs[0].Name)
	}
}

func TestCronService_NilExpiryPreserved(t *testing.T) {
	cs, path := setupService(nil)
	defer os.Remove(path)

	// Simulate a legacy job without expiry by adding and then clearing it
	job, err := cs.AddJob("LegacyJob", CronSchedule{Kind: "every", EveryMS: int64Ptr(60000)}, "msg", "ch", "to")
	if err != nil {
		t.Fatalf("AddJob failed: %v", err)
	}
	job.ExpiresAtMS = nil
	...
	// Run checkJobs — job with nil expiry should NOT be removed
	cs.mu.Lock()
	cs.running = true
	cs.mu.Unlock()
	cs.checkJobs()
	...
	jobs := cs.ListJobs(true)
	if len(jobs) != 1 {
		t.Fatalf("Legacy job with nil expiry should not be removed, got %d jobs", len(jobs))
	}
}

func TestCronService_CustomTTL(t *testing.T) {
	cs, path := setupService(nil)
	defer os.Remove(path)

	at := time.Now().Add(time.Hour).UnixMilli()
	job, err := cs.AddJob("CustomTTL", CronSchedule{Kind: "at", AtMS: &at}, "msg", "ch", "to")
	if err != nil {
		t.Fatalf("AddJob failed: %v", err)
	}

	// Override with custom expiry (7 days)
	customExpiry := time.Now().Add(7 * 24 * time.Hour).UnixMilli()
	job.ExpiresAtMS = &customExpiry
	cs.UpdateJob(job)
	...
	if jobs[0].ExpiresAtMS == nil || *jobs[0].ExpiresAtMS != customExpiry {
		t.Errorf("Custom ExpiresAtMS not preserved after reload")
	}
}
