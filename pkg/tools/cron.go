	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"expires_in_days": map[string]any{
				"type":        "integer",
				"description": "Optional: number of days until this job expires and is auto-deleted. Default: 90.",
			},
...
func (t *CronTool) addJob(ctx context.Context, args map[string]any) *ToolResult {
	...
	// Command jobs are allowed from conversational channels too, but require
	// explicit confirmation when allow_command is disabled. Actual execution is
	// still sandboxed by the exec tool and, for scheduled jobs, runs as an
	// internal system-triggered action in ExecuteJob.
	...
	if expiresInDays, ok := args["expires_in_days"].(float64); ok && expiresInDays > 0 {
		expiresAt := time.Now().UnixMilli() + int64(expiresInDays)*24*60*60*1000
		job.ExpiresAtMS = &expiresAt
		needsUpdate = true
	}
	if needsUpdate {
...
func (t *CronTool) listJobs() *ToolResult {
	...
	for _, j := range jobs {
		...
		expiryInfo := ""
		if j.ExpiresAtMS != nil {
			expiryInfo = fmt.Sprintf(", expires: %s", time.UnixMilli(*j.ExpiresAtMS).Format("2006-01-02"))
		}
		result.WriteString(fmt.Sprintf("- %s (id: %s, %s%s)\n", j.Name, j.ID, scheduleInfo, expiryInfo))
	}
