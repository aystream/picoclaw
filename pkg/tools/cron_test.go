...
// TestCronTool_CommandAllowedFromRemoteChannel verifies command scheduling can be created from remote channels.
func TestCronTool_CommandAllowedFromRemoteChannel(t *testing.T) {
	...
	if result.IsError {
		t.Fatalf("expected command scheduling to succeed from remote channel, got: %s", result.ForLLM)
	}
...
func TestCronTool_ExecuteJobCommandFromRemoteRunsAsSystem(t *testing.T) {
	tool := newTestCronTool(t)
	job := &cron.CronJob{ID: "job-cmd"}
	job.Payload.Channel = "telegram"
	job.Payload.To = "chat-1"
	job.Payload.Command = "printf hello"
	...
	if msg.Channel != "telegram" || msg.ChatID != "chat-1" {
		t.Fatalf("published target = %s/%s, want telegram/chat-1", msg.Channel, msg.ChatID)
	}
	if !strings.Contains(msg.Content, "hello") {
		t.Fatalf("expected command output in published message, got: %s", msg.Content)
	}
	if strings.Contains(msg.Content, "restricted to internal channels") {
		t.Fatalf("expected scheduled command to bypass remote exec restriction, got: %s", msg.Content)
	}
}
